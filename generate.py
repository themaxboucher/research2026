...
def generate_comments_with_model(
    source_code_without_comments: str, model_name: str
) -> dict:
    ...
    for model_name in GENERATE_MODELS:
        try:
            generation = generate_comments_with_model(
                source_code_without_comments, model_name
            )
...
def generate_comments_for_dataset(run_dir: Path, limit: int | None = None) -> None:
    all_files = load_from_jsonl(run_dir, "files")
    ...
    save_to_jsonl(all_files, run_dir, GENERATED_DATASET_FILENAME)
    logging.info(
